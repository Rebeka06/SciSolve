...
namespace SciSolve.Helpers
{
    public static class ExcelExporter
    {
                                           ...
                                           bool converged, string funcExpr)
        {
            ...
            if (steps.Count > 1)
            {
                var chart = ws.AddChart(XLChartType.Line);
                chart.SetTitle($"{method} — Convergence (|error| per iteration)");
                var errorData = steps.Select(s => s.Error).ToList();
                chart.AddDataSeries(
                    Enumerable.Range(1, errorData.Count).Select(i => (object)(double)i),
                    errorData.Select(e => (object)e));
                chart.SetPosition(r + 1, 0, 0, 0);
                chart.SetSize(560, 280);
            }
